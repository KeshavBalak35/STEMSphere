...
from nmrx.model.provenance import EvidenceClass, FieldStatus, IdentityMatch, Lineage
...
def _parse(*payloads):
    """Parse payloads exactly as the shipped (unconfirmed) mapping does. Nothing added."""
...
def _parse_with_provenance(*payloads):
    """Parse, then supply the provenance a CONFIRMED mapping would have carried.

    The shipped mappings cannot state evidence class, lineage or identity match, because no
    real response has been read. This helper stands in for that future confirmed mapping so
    the rest of the pipeline can be exercised.
    """
    records = _parse(*payloads)
    for r in records:
        r.identity_match = IdentityMatch.EXACT
        r.evidence_class = EvidenceClass.MEASURED
        r.source = SourceRef(
            source_id=r.source.source_id, record_id=r.source.record_id, url=r.source.url,
            licence=r.source.licence, lineage=Lineage.ORIGINAL_EXPERIMENT,
        )
    return records


class TestPipeline(unittest.TestCase):
    def test_a_record_from_an_unconfirmed_mapping_is_not_calibration_eligible(self):
        """The shipped mappings cannot state provenance, so their records must not qualify."""
        verdict = evaluate(_parse(COMPLETE)[0])
        self.assertFalse(verdict.eligible)
        self.assertIn("CAL-001", verdict.rule_ids)   # evidence class unknown
        self.assertIn("CAL-002", verdict.rule_ids)   # identity relation unknown
        self.assertIn("CAL-003", verdict.rule_ids)   # lineage unknown

    def test_a_complete_record_survives_the_whole_pipeline(self):
        records = _parse_with_provenance(COMPLETE)
    ...
    def test_a_sparse_record_is_rejected_for_the_right_reasons(self):
        records = _parse_with_provenance(SPARSE)
        verdict = evaluate(records[0])
        self.assertFalse(verdict.eligible)
    ...
    def test_a_sparse_record_is_never_silently_completed(self):
        ...
        record = _parse_with_provenance(SPARSE)[0]
    ...
    def test_mixed_records_are_counted_separately(self):
        report = record_missingness(_parse_with_provenance(COMPLETE, SPARSE))
    ...
    def test_the_dossier_reports_both_findings_and_blocked_sources(self):
        dossier = build(
            ...
            _parse_with_provenance(COMPLETE, SPARSE),
    ...
    def test_two_different_nuclei_are_two_experiments(self):
        clusters = cluster_records(_parse_with_provenance(COMPLETE, SPARSE))
        self.assertEqual(len(clusters), 2)

    def test_the_same_spectrum_from_a_mirror_stays_one_experiment(self):
        records = _parse_with_provenance(COMPLETE, COMPLETE)
